...
const medicineItemSchema = z.object({
  ...
  mrp: z.number().min(0),
  saleRate: z.number().min(0), // This is inclusive of GST
  discount: z.number().min(0),
  taxPercent: z.number().min(0).max(100), // Total GST
  cgstPercent: z.number().optional(), // For storing CGST separately
  sgstPercent: z.number().optional(), // For storing SGST separately
  availableStock: z.number().optional(),
  expiryDate: z.date().optional(),
});

type MedicineItemInput = z.infer<typeof medicineItemSchema>;

interface MedicineFormProps {
  onSubmit: (data: MedicineItemInput) => void;
  onClose: () => void;
  initialData?: Partial<MedicineItemInput>;
}
...
export function BillingMedicineForm({ onSubmit, onClose, initialData }: MedicineFormProps) {
  const [selectedProduct, setSelectedProduct] = useState<ProductWithBatch | null>(null);
  const [selectedBatch, setSelectedBatch] = useState<string | null>(null);

  const form = useForm<MedicineItemInput>({
    resolver: zodResolver(medicineItemSchema),
...
const handleProductSelect = (product: ProductWithBatch) => {
  ...
  if (product.batches && product.batches.length > 0) {
    ...
    form.setValue("taxPercent", product.gst || 0); // Set GST from HSN
  } else {
    form.setValue("productId", product.id);
    form.setValue("productName", product.name);
  ...
  // In BillingMedicineForm, after the tax calculation section:
...
// Total amount is inclusive (since saleRate already includes tax)
const totalAmount = grossAmount - discount;



  const handleFormSubmit = (data: MedicineItemInput) => {
  ...
  // Add CGST and SGST percentages to the data
  onSubmit({
    ...data,
  ...
  return (
    <form onSubmit={form.handleSubmit(handleFormSubmit)} className="space-y-4">
      ...
      {/* Quantity and Pricing */}
        ...
        <div>
            <Label>GST % (Auto from HSN)</Label>
            <Input
            type="number"
            step="0.01"
            max="100"
            {...form.register("taxPercent", { valueAsNumber: true })}
            disabled={!!selectedProduct?.gst}
            />
        </div>
        ...
        <div>
            ...
            <Input 
            value={selectedProduct?.hsnCodeValue || "N/A"} 
            disabled 
        ...
        {/* Tax Breakdown - Shows extraction from inclusive price */}
        {selectedProduct && taxPercent > 0 && (
        <div className="p-3 bg-amber-50 border border-amber-200 rounded-md space-y-2">
            <p className="font-medium text-sm text-amber-900">GST Breakdown (Inclusive)</p>
            ...
            <div className="flex justify-between text-xs font-semibold border-t pt-1">
                <span>Total GST ({taxPercent}%):</span>
